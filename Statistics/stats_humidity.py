...
class Statistics_humidity(Statistics):
    def __init__(self) -> None:
        super().__init__()
        self.__CliendID = "HumidityStatistics"
    ...
    def notify(self,topic,msg): 
        ...
        requests.post('http://127.0.0.1:8080/statistics/Humidity',json.dumps({"value": userID + "-" + PlantID + "-"  + DeviceID + "-" + str(self.value)}))  
        logging.info(f'Sensor of {valuetype} is measuring {self.value} {unit} at time {timestamp} coming on topic {topic} from {client}')
        
    
     
@staticmethod  
def record_humidity(userID, plantID, moist_data): 
    values =  [[],[]]
...
if __name__ == "__main__":
        ...
        topics = (requests.get('http://127.0.0.1:8080/catalog/all_topics?program=Statistics&type=temperature').json())["value"]

        Statistics = Statistics_humidity(broker , port)
        Statistics.start()
        for topic in topics: 
